...
**Both full codes (`10.71C`) and prefixes (`10.71`, `10`) are accepted.**
When you are not sure of the sub-letter, return only the prefix — the
search expands a 5-char prefix to every NAF sub-code under it. Do not
guess sub-letters. NAF uses A/B/C/D suffixes, not the European NACE `Z`
suffix; if the input names an activity but you cannot confidently pick
one of A/B/C/D, give the 5-char prefix instead.

Pick the most specific code(s) consistent with the description. Two or
three codes (or one prefix) are normal; ten codes means you're guessing.
...
arrondissements or communes, also set `postal_codes` — full codes like
`75001` or prefixes like `06` for a whole department area.
